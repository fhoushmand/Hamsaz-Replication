...
public class FloodingConsensusSession extends Session {
  ...
  private void handleMySet(MySetEvent event) {
    Debug.print("FC: receiving a new set: " + event.toString());
	  ...
	  HashSet<Proposal> set = (HashSet<Proposal>) event.getMessage().popObject();
  ...
  private void handleDecided(DecidedEvent event) {
    Debug.print("FC: decide event: " + event.toString());
    ...
    try {
      DecidedEvent ev = new DecidedEvent(event.getChannel(), Direction.DOWN,
          this);
      ev.getMessage().pushObject(decided);
      ev.go();
    } catch (AppiaEventException ex) {
