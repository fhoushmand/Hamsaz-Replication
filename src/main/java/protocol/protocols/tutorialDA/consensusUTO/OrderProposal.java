...
import main.java.protocol.protocols.tutorialDA.consensusUtils.Proposal;
...
public class OrderProposal extends Proposal {
   ...
   * 
   * number_of_msg+(seq+intClassSize+className+port+intSourceSize+source+intMsgSize+msg)*number_of_msg
   * number_of_msg+(seq+intClassSize+className+port+intSourceSize+source+intMsgSize+msg+int+mtobPayloadBytes)*number_of_msg
   * 
   */
  public byte[] bytes;
  MTOBPayload payload;
  LinkedList<ListElement> auxObj;

//  public OrderProposal(byte[] bytesm, LinkedList<ListElement> ao) {
//    this.bytes = bytesm;
//    this.auxObj = ao;
//  }
  public OrderProposal(byte[] bytesm, LinkedList<ListElement> ao, MTOBPayload p) {
    this.bytes = bytesm;
    this.auxObj = ao;
    this.payload = p;
  }

  public OrderProposal(byte[] bytesm) {
    this.bytes = bytesm;
  }
  ...
  public int compareTo(Proposal other) {
    ...
    for (int g = 0; g < nElem_this; g++) {
      ...
      curPos_other += 4;
      // source
      String host_this = new String(bytes, curPos_this + 4, byteArrayToInt(
          bytes, curPos_this));
      curPos_this += byteArrayToInt(bytes, curPos_this) + 4;
      ...
      int this_payload_size = byteArrayToInt(bytes, curPos_this);
      int other_payload_size = byteArrayToInt(bytes_other, curPos_other);

      curPos_this += this_payload_size + 4;
      curPos_other += other_payload_size + 4;
